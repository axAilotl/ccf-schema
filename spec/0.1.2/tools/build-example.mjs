...
addRecord(ids.runtime, 'core.runtime', 3, { person_id: ids.person, recorded_by: ids.runtime, recorded_at: '2026-08-11T21:40:00.000Z', policy_ref: policyRef, authority: { basis: 'runtime_import', asserted_by: ids.runtime, accepted_by: null }, payload: { kind: 'backend', name: 'Personal archive adapter', version: '0.1.2-example', instance_id: 'personal-local', capabilities: ['capture','transcribe','extract','sync'], operator_id: ids.person, extensions: {} }, extensions: {} });
...
const linkSubs = [
  ...
  linkSub(ids.generatedBy,'ccf.generated_by',ids.utterance,ids.run), linkSub(ids.evidenceFor,'ccf.evidence_for',ids.utterance,ids.candidate,{kind:'text_span',start:0,end:59}),
...
fs.writeFileSync(path.join(EX,'README.md'),'# Personal archive trace\n\nBootstrap an archive, then admit one signed offline capture batch containing a source, session, audio Blob, artifact, transcription run, utterance, candidate preference, human review, accepted preference, and provenance Links.\n');
