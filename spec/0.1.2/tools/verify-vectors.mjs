...
const suppressionKey=Buffer.from(suppressionVector.key_hex,'hex');
ok(suppressionContentDigest(suppressionVector.content_fixture.canonical_plaintext)===suppressionVector.content_fixture.expected_content_digest,'stable suppression content digest');
ok(suppressionScopeCommitment(suppressionVector.scope_object_ids)===suppressionVector.expected_scope_commitment,'suppression scope commitment');
for(const c of suppressionVector.rejected_preimages){let rejected=false;try{c.value.kind==='origin'?suppressionOriginToken(suppressionKey,c.value):suppressionContentToken(suppressionKey,c.value);}catch{rejected=true;}ok(rejected,`reject suppression preimage ${c.name}`);}
